...
import { ArrowLeft, PlusCircle, Sparkles, Trash2, Bell, Zap, Loader2 } from 'lucide-react';
...
export default function EditEventPage() {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      ...
      if (snapshot.exists()) {
        const data = snapshot.val() as Omit<LuckyEvent, 'id'>;
        const fetchedEvent = { id: eventId, ...data };
        setEvent(fetchedEvent);
        setEventName(fetchedEvent.name);
        setStartTime(format(new Date(fetchedEvent.startTime), "yyyy-MM-dd'T'HH:mm"));
        setEndTime(format(new Date(fetchedEvent.endTime), "yyyy-MM-dd'T'HH:mm"));
        setResultTime(format(new Date(fetchedEvent.resultTime), "yyyy-MM-dd'T'HH:mm"));
        setCodes(fetchedEvent.codes || ['']);
        setSelectionMode(fetchedEvent.selectionMode);
        setIsHighlighted(fetchedEvent.isHighlighted || false);
        setSendNotification(fetchedEvent.sendNotification || false);
        setRequiredXp(fetchedEvent.requiredXp);
        
        if (fetchedEvent.selectionMode === 'custom' && fetchedEvent.customWinnerSlots) {
            const codeToIndexMap: Record<string, number> = {};
            fetchedEvent.codes.forEach((code, index) => {
                codeToIndexMap[code] = index;
            });
            const slots: Record<number, number> = {};
            Object.entries(fetchedEvent.customWinnerSlots).forEach(([code, slot]) => {
                const index = codeToIndexMap[code];
                if(index !== undefined) {
                    slots[index] = slot;
                }
            });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updatedEvent: Partial<Omit<LuckyEvent, 'id'>> = {
      name: eventName,
      ...
      customWinnerSlots: selectionMode === 'custom' ? customWinnerSlots : {},
    ...
    if(!requiredXp || requiredXp <=0) {
        delete updatedEvent.requiredXp;
    }
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(value: 'custom' | 'random') => setSelectionMode(value)} value={selectionMode}>
              <SelectTrigger>
